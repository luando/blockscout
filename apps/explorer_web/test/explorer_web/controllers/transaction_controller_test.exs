defmodule ExplorerWeb.TransactionControllerTest do
  ...
  describe "GET show/3" do
    test "redirects to transactions/:transaction_id/token_transfers when there are token transfers", %{conn: conn} do
      ...
      conn = get(conn, transaction_path(ExplorerWeb.Endpoint, :show, locale, transaction))

      assert redirected_to(conn) =~ transaction_token_transfer_path(ExplorerWeb.Endpoint, :index, locale, transaction)
    end
    ...
    test "redirects to transactions/:transaction_id/internal_transactions when there are no token transfers", %{
      ...
      assert redirected_to(conn) =~
               transaction_internal_transaction_path(ExplorerWeb.Endpoint, :index, locale, transaction)
    end
  end
end
