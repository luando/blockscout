defmodule ExplorerWeb.TransactionTokenTransferControllerTest do
  ...
  import ExplorerWeb.Router.Helpers, only: [transaction_token_transfer_path: 4]
  ...
  describe "GET index/3" do
    test "load token transfers", %{conn: conn} do
      ...
      conn = get(conn, transaction_token_transfer_path(ExplorerWeb.Endpoint, :index, :en, transaction.hash))
    ...
    test "with missing transaction", %{conn: conn} do
      ...
      conn = get(conn, transaction_token_transfer_path(ExplorerWeb.Endpoint, :index, :en, hash))

      assert html_response(conn, 404)
    end

    test "with invalid transaction hash", %{conn: conn} do
      conn = get(conn, transaction_token_transfer_path(ExplorerWeb.Endpoint, :index, :en, "nope"))
    ...
    test "includes token transfers for the transaction", %{conn: conn} do
      ...
      path = transaction_token_transfer_path(ExplorerWeb.Endpoint, :index, :en, transaction.hash)
    ...
    test "returns next page of results based on last seen token transfer", %{conn: conn} do
      ...
      conn =
        get(conn, transaction_token_transfer_path(ExplorerWeb.Endpoint, :index, :en, transaction.hash), %{
