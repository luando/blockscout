defmodule Explorer.Chain.Token do
  ...
  * `:contract_address_hash` - Address hash foreign key
  ...
  @doc """
  Build a `Ecto.Query` to fetch tokens that the given address has interacted with.

  In order to fetch a token, the given address must be transfered tokens to or received tokens
  from an another address.
  """
  def from_address(address_hash) do
    from(
      token in Token,
